CREATE TABLE IF NOT EXISTS schema_migrations (
    ...
    name TEXT NOT NULL,
    applied_at TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"+00:00"'))
);
...
CREATE TABLE IF NOT EXISTS users (
    ...
    created_at TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"+00:00"')),
    updated_at TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"+00:00"'))
);
...
CREATE INDEX IF NOT EXISTS idx_instance_members_created_by
ON instance_members(created_by_user_id);

CREATE OR REPLACE FUNCTION reject_instance_owner_member()
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'instance owner cannot be a member';
    END IF;
    RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS prevent_instance_owner_member_insert ON instance_members;
CREATE TRIGGER prevent_instance_owner_member_insert
BEFORE INSERT OR UPDATE OF instance_id, user_id ON instance_members
FOR EACH ROW EXECUTE FUNCTION reject_instance_owner_member();

CREATE OR REPLACE FUNCTION reject_instance_member_owner_transfer()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'instance member must be removed before ownership transfer';
    END IF;
    RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS prevent_instance_member_becoming_owner ON instances;
CREATE TRIGGER prevent_instance_member_becoming_owner
BEFORE UPDATE OF owner_user_id ON instances
FOR EACH ROW EXECUTE FUNCTION reject_instance_member_owner_transfer();
...
CREATE INDEX IF NOT EXISTS idx_operation_records_actor_user_id ON operation_records(actor_user_id);
...
CREATE INDEX IF NOT EXISTS idx_execution_jobs_parent_request_id
ON execution_jobs(parent_request_id);
CREATE INDEX IF NOT EXISTS idx_execution_jobs_actor_user_id
ON execution_jobs(actor_user_id);
...
CREATE TABLE IF NOT EXISTS activity_snapshots (
    ...
    error_summary TEXT,
    collected_at TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"+00:00"')),
    FOREIGN KEY (instance_id) REFERENCES instances(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_hermes_auth_grants_client_instance
ON hermes_auth_grants(client_id, instance_id);
CREATE INDEX IF NOT EXISTS idx_hermes_auth_grants_instance_id
ON hermes_auth_grants(instance_id);
CREATE INDEX IF NOT EXISTS idx_hermes_auth_grants_user_id
ON hermes_auth_grants(user_id);
CREATE INDEX IF NOT EXISTS idx_hermes_auth_grants_manager_session_id
ON hermes_auth_grants(manager_session_id);

INSERT INTO schema_migrations (version, name)
VALUES (8, 'hermes_auth_bridge') ON CONFLICT (version) DO NOTHING;

INSERT INTO schema_migrations (version, name)
VALUES (3, 'local_auth_session') ON CONFLICT (version) DO NOTHING;

INSERT INTO schema_migrations (version, name)
VALUES (4, 'control_plane_model') ON CONFLICT (version) DO NOTHING;

INSERT INTO schema_migrations (version, name)
VALUES (5, 'instance_provisioning') ON CONFLICT (version) DO NOTHING;

INSERT INTO schema_migrations (version, name)
VALUES (6, 'external_session_tokens') ON CONFLICT (version) DO NOTHING;

INSERT INTO schema_migrations (version, name)
VALUES (7, 'activity_snapshots') ON CONFLICT (version) DO NOTHING;
