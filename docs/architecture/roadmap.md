...
- Resolve per-user upstreams through Docker DNS at request time, so container restarts do not leave instances behind a 502 when their IP changes.
- 通过 Docker DNS 在请求时解析每用户上游，避免容器重启后 IP 变化导致实例访问 502。
