...
- 管理员实例列表和使用成效列表在桌面端使用可用内容宽度，并在宽表格顶部和底部提供同步横向滚动。
...
管理员可在平台用户管理、批量创建实例、批量设置模型和设备审批的上传区域下载 CSV 模板。
平台用户分别提供 UIS 和 Local 模板，共五种模板。下载沿用管理员权限及内部请求鉴权。

模板使用 UTF-8 BOM 编码，包含表头和一行虚拟示例；导入前必须替换或删除示例行。
UIS 状态支持 `active`（启用）、`disabled`（停用）、`locked`（锁定），示例使用 `disabled`。
Local 示例密码必须替换为独立密码。实例模板使用 Hermes 示例，Owner 必须是已导入并启用的身份；
模型模板的 `user_id` 是实例的兼容 ID，地址和密钥兼容列留空；设备审批模板填写实例 UUID。
下载本身不创建用户、实例或任务。旧版已退役页面不新增下载入口。
