...
class MetadataConsistencyTests(unittest.TestCase):
    ...
    def test_active_hermes_client_requires_provider_files_and_private_env(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            self.assertEqual(
                {issue.code for issue in reporter.issues},
                {"hermes_auth_provider_not_enabled", "hermes_auth_provider_env_invalid"},
            )

    def test_active_hermes_client_requires_container_readable_provider_permissions(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            db_file = root / "manager.db"
            data_path = root / "hermes"
            ...
            plugin.chmod(0o770)
            (plugin / "plugin.yaml").chmod(0o660)
            (plugin / "__init__.py").chmod(0o660)
